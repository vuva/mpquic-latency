...
pathLoop:
	...
	if highestRate != 0 && next_stream != nil {
		...
		if shouldRedundant {
			...
			} else if availablePathCount == 1 && selectedPath.pathID != highestRatePath.pathID && !selectedPath.SendingAllowedWithReserved(1000*protocol.MaxPacketSize) {
...
func (sch *scheduler) sendPacket(s *session) error {
	...
	for {
		...
		utils.Debugf("\n Ninetails: pktsent %d at path %d sendrate %d quota %d", pkt.PacketNumber, pth.pathID, pth.rttStats.GetSendRate(), sch.quotas[pth.pathID])

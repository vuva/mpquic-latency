...
func (sch *scheduler) getRetransmission(s *session) (hasRetransmission bool, retransmitPacket *ackhandler.Packet, pth *path) {
	...
	retransmitLoop:
		...
		utils.Debugf("\t At %d Dequeueing retransmission of packet %d from path %d", time.Now().UnixNano(), retransmitPacket.PacketNumber, pth.pathID)
...
pathLoop:
	...
	if highestRate != 0 {
		shouldRedundant = dataInStream <= 3*uint64(protocol.MaxPacketSize) || float64(dataInStream*8)/float64(highestRate)*1000.0 < float64(utils.Min(int(3*highestRatePathRTT/2), int(uint64(minRTOTimeout)+highestRatePathRTT/2)))
		utils.Debugf("\n Ninetails: selectedPathID %d availablepaths %d \n highestRatepath %d = %d bps, %d ms; \n lowRTTpath %d = %d bps, %d ms \n stream datainstream %d with %f >< %d + %d/2 ,shouldRedundant %t", selectedPath.pathID, availablePathCount, highestRatePath.pathID, highestRate, highestRatePathRTT, lowestRTTPath.pathID, lowestRTTPathRate, lowestRTT, dataInStream, float64(dataInStream*8)/float64(highestRate)*1000.0, highestRatePathRTT, lowestRTT, shouldRedundant)
		if shouldRedundant {
...
func (sch *scheduler) performPacketSending(s *session, windowUpdateFrames []*wire.WindowUpdateFrame, pth *path) (*ackhandler.Packet, bool, error) {
	...
	for _, f := range windowUpdateFrames {
		s.packer.QueueControlFrame(f, pth)
		utils.Debugf("\n QueueControlFrame: path %d stream %d byteoffset %d at %d", pth.pathID, f.StreamID, f.ByteOffset, time.Now().UnixNano())
	}

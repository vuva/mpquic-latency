...
func (pcm *pconnManager) run() {
	...
	// Start the timer for periodic interface checking (only for server)
...
// Return wether the IP is a local address
func isLinkLocal(ip net.IP) bool {
	if ip4 := ip.To4(); ip4 != nil {
		return ip4[0] == 10 || // classA
			(ip4[0] == 172 && ip[1] == 16) || // classB
			(ip4[0] == 169 && ip4[1] == 254) || // link local addresses
			(ip4[0] == 192 && ip4[1] == 168) // classC
	}

	//return ip[0] == 0xfe // IPv6 local link address 0xfe80.. to 0xfebf..
	return false
}
...
func (pcm *pconnManager) createPconns() error {
	...
	for _, i := range ifaces {
		...
		if !strings.Contains(i.Name, "eth") && !strings.Contains(i.Name, "rmnet") &&
			!strings.Contains(i.Name, "wlan") && !strings.Contains(i.Name, "enp") {
			continue
		}
		addrs, err := i.Addrs()
		if err != nil {
		...
		for _, a := range addrs {
			...
			// Exclude public addresses and IPv6 addresses
			if !isLinkLocal(ip) {
				continue
			}

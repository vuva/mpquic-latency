...
//VUVA Ninetails
func (h *sentPacketHandler) SendingAllowedWithReserved(reservedBytes protocol.ByteCount) bool {
	congestionLimited := h.bytesInFlight > h.congestion.GetCongestionWindow()-reservedBytes
	maxTrackedLimited := protocol.PacketNumber(len(h.retransmissionQueue)+h.packetHistory.Len()) >= protocol.MaxTrackedSentPackets
	if congestionLimited {
